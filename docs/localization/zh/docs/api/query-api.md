...
|    属性             | 类型     | 最大长度 | 必填    | 是否签名   | 说明                          |
| :---------: | -------- | -------- | ---- | -------- | :---------------------------- |
| actionResults               | `json[]`   |        | N    |        | action执行结果(合约发布和执行成功会返回值)                      |

- 合约actionResults结构

|    属性             | 类型     | 最大长度 | 必填    | 是否签名   | 说明                          |
| :---------: | -------- | -------- | ---- | -------- | :---------------------------- |
| method               | `string`   |        | N    |        | 合约执行方法                      |
| result               | `string`   |        | N    |        | 合约执行方法返回的结果                      |
| logInfoList          | `json[]`   |        | N    |        | 合约执行日志记录                      |

- logInfoList结构

|    属性             | 类型     | 最大长度 | 必填    | 是否签名   | 说明                          |
| :---------: | -------- | -------- | ---- | -------- | :---------------------------- |
| address               | `string`   |        | N    |        | 当前合约的地址                      |
| data               | `string`   |        | N    |             | 合约日志内容                      |
| topics          | `json[]`   |        | N    |        | 日志标签，第一个标签为日志事件hash值                      |

```json tab="actionResults数据格式"
{
	{
     "method":"",
     "result":"",
